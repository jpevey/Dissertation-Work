...
import Run_CNN_GA
#GA_calc = Run_CNN_GA.ga_object()
#GA_calc.load_CNN_GA_Options()
#GA_calc.run_GA(1, [], [])
#exit()

class genetic_algorithm:
    def __init__(self):
        print("Created genetic algorithm object")
        self.last_time_value = 0

    def timeit_function(self, step_str):
        if self.options['writeout_timeit_file']:
            with open(self.options['timeit_file_string'], 'a') as write_file:
                time_value = time.time() - self.time_0
                write_file.write("{}&{}_{}&{}&{}\n".format(self.generation,self.generation, step_str, time_value, (self.last_time_value - time_value)))
                self.last_time_value = time_value


    def run_GA_calculation(self, options_dict):
        ...
        ### Initializing timeit file
        with open(self.options['timeit_file_string'], 'w') as timeit_file:
            timeit_file.write("generation, description, time (s)\n")
        ...
        self.timeit_function('initialize')
        ...
        self.timeit_function('build initial population')
        ...
        if self.options['include_pattern']:
            ...
            for pattern_per_ind in self.options['pattern_to_include']:
                for ind_count, pattern_to_include in enumerate(self.options[pattern_per_ind]):
                    print(ind_count, self.options['number_of_parents'])
                    #print(getattr(self.individuals[ind_count], pattern_per_ind))
                    if ind_count >= int(self.options['number_of_parents']):
                        continue

                    #print(len(self.individuals), ind_count)
                    setattr(self.individuals[ind_count-1], pattern_per_ind, pattern_to_include)
                    #print(getattr(self.individuals[ind_count], pattern_per_ind))
            ...
            self.timeit_function('include pattern')
        ...
        if self.options['write_output_csv']:
            ...
            output_csv.close()
            self.timeit_function('write initial output csv')
        ...
        for constraint in self.options['constraint']:
            if 'preevaluate' in constraint:
                self.individuals = self.apply_constraint(self.individuals, constraint)
        self.timeit_function('preevaluate constraint')

        print("Evaluating initial population")
        self.individuals = self.evaluate(self.options['fitness'], self.individuals)
        self.timeit_function('initial evaluation')

        for constraint in self.options['constraint']:
            if 'postevaluate' in constraint:
                self.individuals = self.apply_constraint(self.individuals, constraint)
        self.timeit_function('postevaluate constraint')


        self.all_individuals = copy.deepcopy(self.individuals)
        #print(len(self.all_individuals))
        self.timeit_function('copying all individuals')

        ### Updating networks from first iterations
        if self.options['update_cnn']:
            self.update_cnn()
        self.timeit_function('initial cnn training')

        if self.options['use_hashtable_for_duplicate_check']:
            self.hash_list = self.hash_individuals(self.all_individuals)
            self.timeit_function('hashing initial individuals')

        if self.options['use_non_dominated_sorting'] == True:
            self.parents_list = self.non_dominated_sorting()
            self.timeit_function('initial nondominated sorting')
        else:
        ...
        if self.options['choose_parent_based_on_bitwise_diversity']:
            ...
            self.timeit_function('choose_parent_based_on_bitwise_diversity')

        self.write_output_v2(self.individuals)
        self.timeit_function('write out initial individuals')
        self.generation += 1
        ...
        for generation in range(self.options['number_of_generations']):
            ...
            if self.options['use_recursive_cnn_ga_solver']:
                print("Using recursive GA/CNN solver.")

                print("Starting CNN solver with ")
                GA_calc = Run_CNN_GA.ga_object()
                GA_calc.load_CNN_GA_Options()
                self.timeit_function('initialize recursive GA')
                #if self.options['use_non_dominated_sorting'] == True:
                #    self.parents_list = self.non_dominated_sorting()
                 #   self.timeit_function('nondom sorting')
                #else:
                #    self.individuals.sort(key=lambda x: getattr(x, self.options['fitness_sort_by']), reverse=True)
                #    ### Pairing down individuals to be specified number
                #    self.individuals = self.individuals[:self.options['number_of_individuals']]
                #    self.timeit_function('sorting')
                ### building option override dict
                override_dictionary = {}
                override_dictionary['CNN_solver_objective_functions'] = self.options['CNN_solver_objective_functions']
                override_dictionary['timeit_file_string'] = "timeit_rec_ga_{}.csv".format(self.generation)

                constraint_split = self.options['constraint'][0].split("#")
                constraint_options = constraint_split[3]
                constraint_options_split = constraint_options.split(",")
                linearSimAnneal_threshold, relaxed_constraint_Bool = self.get_linearsimanneal_values(
                    constraint_options_split)
                temp_ = 'representativity#postevaluate#linearSimAnneal#variable,gen_10,val_%REPLACE%,ensureParentPop_True'
                temp_ = temp_.replace("%REPLACE%", str(linearSimAnneal_threshold))
                override_dictionary['constraint'] = [self.options['constraint'][0], temp_]
                list_of_mutated_children = GA_calc.run_GA(self.generation,
                                                          self.build_matrial_matrix(self.parents_list, 'material_matrix'),
                                                          self.build_matrial_matrix(self.parents_list, 'material_matrix_cassette_2A'),
                                                          override_dictionary)
                self.timeit_function('run recursive GA calculation')

                list_of_mutated_children = self.remake_duplicate_children_hash(list_of_mutated_children)
                self.timeit_function('check for duplicates in final REC pareto')

                for ind_ in list_of_mutated_children:
                    ind_.update_input_file_strings(self.generation, self.individual_count)
                    setattr(ind_, "number_of_plates_in_cassette_2A", len(getattr(ind_, "material_matrix_cassette_2A")))
                    self.individual_count+=1
                self.timeit_function('Setting individual parameters')

                ### Applying constraints to the population returned by the recursive cnn solver
                for constraint in self.options['constraint']:
                    if "linearSimAnneal" in constraint:
                        print("Applying linearSimAnneal to all previous individuals")
                        list_of_mutated_children = self.apply_constraint(list_of_mutated_children, constraint)
                    self.timeit_function('checking constraint {}'.format(constraint))
                ...
                self.timeit_function('crossover')
                print("mutating")
                ...
                if self.options['CNN_check_mutation_bool']:
                    ...
                    self.timeit_function('CNN_check_mutation_bool')
                    ...
                    self.timeit_function('mutate')
                    if self.options['adjustable_zone_2A_cassette_bool']:
                        list_of_mutated_children = self.mutate(list_of_mutated_children,
                                                               'material_matrix_cassette_2A',
                                                               do_variable_size_mutation = True)
                        self.timeit_function('adjustable zone 2a mutate')
                ...
                if self.options['remake_duplicate_children']:
                    ...
                    #                                                          debug = False)
                    self.remake_duplicate_children_hash(list_of_mutated_children)
                    self.timeit_function('remake duplicate children')
                ...
                if self.options['enforce_material_count_before_evaluation']:
                    ...
                    self.timeit_function('enforce material count pre eval')

            for constraint in self.options['constraint']:
                if 'preevaluate' in constraint:
                    list_of_mutated_children = self.apply_constraint(list_of_mutated_children, constraint)
            self.timeit_function('preevaluate constraint list_of_mutated_children')

            print("Evaluating list_of_mutated_children")
            evaluated_children = self.evaluate(self.options['fitness'], list_of_mutated_children)
            self.timeit_function('evaluation list_of_mutated_children')

            for constraint in self.options['constraint']:
                if 'postevaluate' in constraint:
                    list_of_mutated_children = self.apply_constraint(list_of_mutated_children, constraint)
            self.timeit_function('postevaluate constraint list_of_mutated_children')

           # print("evaluating children")
           # evaluated_children = self.evaluate(self.options['fitness'], list_of_mutated_children)

            #self.timeit_function('evaluate')
            ### Adding now evaluated children to master list of all individuals
            self.all_individuals += evaluated_children
            self.hash_list += self.hash_individuals(evaluated_children)
            ...
            if self.options['CNN_check_keff_adjustable_bias']:
                if self.options['CNN_check_keff_crossover'] or self.options['CNN_check_keff_mutation']:
                    ...
                    self.timeit_function('CNN_check_keff_crossover')
                self.timeit_function('CNN_check_keff_adjustable_bias')
            if self.options['update_cnn']:
                self.update_cnn()



            ### Sorting this generation's individuals before outputting csv

            if self.options['use_non_dominated_sorting'] == True:
                self.parents_list = self.non_dominated_sorting()
                self.timeit_function('non dom parent sort')
            else:
                self.individuals.sort(key=lambda x: getattr(x, self.options['fitness_sort_by']), reverse=True)
                ### Pairing down individuals to be specified number
                self.individuals = self.individuals[:self.options['number_of_individuals']]
                self.timeit_function('parent sort')
            ...
            if self.options['write_output_csv']:
                if self.options['sort_all_possible_individuals']:
                    ...
                    self.write_output_v2(self.individuals)
                self.timeit_function('write output')

            self.generation += 1
            self.cnn_handler.clear_models_from_memory()
            self.cleanup_folder(['.inps', '.sh'], ['.sh.'])
        ...
        if self.options['output_all_individuals_at_end_of_calculation'] == True:
            ...
            self.timeit_function('final writeout individuals')
        if 'return_parents' in self.options:
            if self.options['return_parents']:
                return self.parents_list


    def cleanup_folder(self, list_of_extensions, list_in_file_name):
        for file_extension in list_of_extensions:
            for file in os.listdir():
                if file.endswith(file_extension):
                    os.remove(file)
        for file_flag in list_in_file_name:
            for file in os.listdir():
                if file_flag in file:
                    os.remove(file)

    def update_cnn(self):
        ### This function seperates individuals into training and validation sets
        self.broad_train, self.broad_validation, self.narrow_train, self.narrow_validation = self.cnn_handler.preprocess_cnn_trainval_individuals(
            individuals=self.all_individuals,
            validation_split=self.options['CNN_training_validation_split'])
        print("self.broad_train, self.broad_validation, self.narrow_train, self.narrow_validation",
              len(self.broad_train), len(self.broad_validation), len(self.narrow_train), len(self.narrow_validation))
        ### Updating Neural Networks
        for network in self.options['update_cnn_options']:
            print("Updating CNN network {}".format(network))

            ### Building training and validation datasets
            training_data_x = self.cnn_handler.build_list_of_fns_patterns_for_cnn_3x3(
                getattr(self, self.options['update_cnn_options'][network]['type'] + "_train"))
            training_data_y = self.cnn_handler.get_y_values(
                getattr(self, self.options['update_cnn_options'][network]['type'] + "_train"), network)
            validation_data_x = self.cnn_handler.build_list_of_fns_patterns_for_cnn_3x3(
                getattr(self, self.options['update_cnn_options'][network]['type'] + "_validation"))
            validation_data_y = self.cnn_handler.get_y_values(
                getattr(self, self.options['update_cnn_options'][network]['type'] + "_validation"), network)

            ### skipping retraining if there are not more than 10 validation examples to train on for this model
            ###
            if len(validation_data_x) < self.options['CNN_training_minimum_validation_count']:
                print("Not enough validation data to re-train {} model. Continuing.".format(network))
                continue

            ### Load and train model
            new_model_string = self.cnn_handler.train_network(network_type=network,
                                                              network_string=
                                                              self.options['CNN_solver_objective_functions'][network],
                                                              x_train=training_data_x,
                                                              y_train=training_data_y,
                                                              x_val=validation_data_x,
                                                              y_val=validation_data_y,
                                                              batch_size=int(len(training_data_x) / 2),
                                                              epochs=self.options['update_cnn_options'][network][
                                                                  'epochs'],
                                                              generation=self.generation)

            print("New network: {}".format(new_model_string))
            split_ = new_model_string.split(".hdf5")
            self.options['CNN_solver_objective_functions'][network] = os.path.abspath(split_[0])
            self.timeit_function('update cnn {}, {}'.format(network, new_model_string))

    def build_matrial_matrix(self, individuals, target_string):
        return_list = []
        for ind in individuals:
            return_list.append(getattr(ind, target_string))
        return return_list


    def hash_individuals(self, individuals):
        hashes = []
        for individual in individuals:
            individual_string = str(individual.make_material_string_csv('material_matrix_cassette_2A',
                                                'material_matrix_cassette_2A_str'))
            individual_string += str(individual.make_material_string_csv('material_matrix', 'material_string_csv'))
            hash_object = hashlib.md5(individual_string.encode())
    ...
    def apply_cnn_mutation_heuristic(self, list_of_children):
        ...
        np_array_of_all_children_inputs = self.cnn_handler.build_list_of_fns_patterns_for_cnn_3x3(total_list_of_mutated_children)
  ...
  #              setattr(individual,  objective_function + "_cnn", prediction[0])"""
    ...
    def apply_constraint(self, list_of_individuals, constraint_type):
        ...
        if constraint_type_ == 'keff':
            if 'mcnp' in self.options['solver']:
                ...
                for individual in list_of_individuals:
                    ...
                    self.mcnp_file_handler.write_mcnp_input(
                        template_file_str=self.options['mcnp_keff_template_file_string'],
            ...
            if 'cnn' in self.options['solver']:
                ### Building list of input variables
                np_array_of_all_individual_inputs = self.cnn_handler.build_list_of_fns_patterns_for_cnn_3x3(list_of_individuals)
        ...
        if constraint_type_ == 'representativity':
            ...
            if constraint_lin_sim_anneal == "linearSimAnneal":
                linearSimAnneal_threshold, relaxed_constraint_Bool = self.get_linearsimanneal_values(constraint_options_split)
                ...
                if relaxed_constraint_Bool == 'True':
                    ...
                    while number_meeting_constraint < self.options['number_of_parents']:
                        ...
                        if linearSimAnneal_threshold < 0.0:
                            print("{} individuals meet the <0.0 constraint on {}, continuing".format(number_meeting_constraint, linearSimAnneal_threshold))
                            break
                    self.linearSimAnneal_threshold = linearSimAnneal_threshold
    ...
    def get_linearsimanneal_values(self, constraint_options_split):
        ...
        return linearSimAnneal_threshold, relaxed_constraint_Bool
    ...
    def non_dominated_sorting(self,
        ...
        if individuals_to_sort == 'default':
            ...
            if self.options['sort_all_possible_individuals']:
                ...
                for constraint in self.options['constraint']:
                    if 'postevaluate' in constraint:
                        individuals_to_sort = self.apply_constraint(individuals_to_sort, constraint)
...
### Checks if ind_1 dominates ind_2
    ...
    def remake_duplicate_children_hash(self, list_of_children, debug = True):

        ### Hashing all child individuals
        child_hash_list = self.hash_individuals(list_of_children)

        ### Comparing those hashed values to the master list, mutating those that are already in the list
        for child, hash_value in zip(list_of_children, child_hash_list):
            for ind_hash_value in self.hash_list:
                if hash_value == ind_hash_value:
                    child.material_matrix = self.single_bit_mutation(child.material_matrix, force_mutation=True,
                                                                     force_mutation_per_material_sublist=2)
    ...
    def remake_duplicate_children(self, list_of_children, comparison_list, debug = True):
        ...
        for child in list_of_children:
            ...
            if self.options['material_override_boolean']:
    ...
    def evaluate(self, evaluation_types, list_of_individuals):

        print(len(list_of_individuals))
        ...
        for evaluation_type in evaluation_types:
            ...
            if evaluation_type == 'representativity':
                ...
                if 'cnn' in self.options['solver']:
                    ...
                    for individual in list_of_individuals:
                        individual.representativity_cnn = individual.representativity
                ...
                if 'cnn' in self.options['solver']:
                    ...
                    for individual in list_of_individuals:
                        if individual.acceptable_eigenvalue == False:
                            ...
                            individual.total_flux_cnn = individual.total_flux
            if evaluation_type == 'integral_keff':
                ...
                if 'cnn' in self.options['solver']:
                    ...
                    for ind_ in list_of_individuals:
                        ...
                        ind_.integral_keff_cnn = ind_.integral_keff_value
        ...
        if 'mcnp' in self.options['solver']:
            for individual in list_of_individuals:
                ...
                if self.options['fake_fitness_debug'] == True:
                    ...
                    for evaluation_type in evaluation_types:
                        individual = self.mcnp_evaluation_calculation(individual, evaluation_type)
    ...
    def mcnp_evaluation_calculation(self, individual, evaluation_type):
        if evaluation_type == 'representativity':
            if individual.acceptable_eigenvalue == True:
                ...
                individual.representativity = self.mcnp_file_handler.calculate_representativity_v3(
                    individual.flux_values, lethargy_adjustment=self.options['Lethargy_Adjustment_of_Rep'],
                    bins=self.options['energy_bins'])
            if individual.acceptable_eigenvalue == False:
                individual.representativity = 0.0
        ...
        if evaluation_type == 'integral_keff':
            if individual.acceptable_eigenvalue == True:
                individual.integral_keff_value = self.mcnp_file_handler.get_keff(
                    individual.integral_keff_input_file_string)
    ...
    def crossover(self, keff_constraint_string = 'keff'):
        ...
        for new_child_value in range(number_of_children):
            ...
            if random.uniform(0, 1.0) <= self.options['crossover_rate']:
            ...
            ### Mutating a parent to create new individual
            else:
                parent_ = self.parents_list[random.randint(0, self.options['number_of_parents'] - 1)]
                new_child_ind = individual.individual(self.options, self.generation, self.individual_count)
                self.individual_count += 1

                setattr(new_child_ind, "material_matrix_cassette_2A", copy.deepcopy(getattr(parent_, "material_matrix_cassette_2A")))
                setattr(new_child_ind, "material_matrix", copy.deepcopy(getattr(parent_, "material_matrix")))


                ### Mutating child
                original_material_matrix    = getattr(new_child_ind, "material_matrix")
                original_material_matrix_2a = getattr(new_child_ind, "material_matrix_cassette_2A")
                ### Doing single bit mutation
                setattr(new_child_ind, "material_matrix", self.single_bit_mutation(original_material_matrix, mutating_debug = False))
                setattr(individual, "material_matrix_cassette_2A", self.variable_cassette_length_mutation(original_material_matrix_2a, mutating_debug = False))

                ### Resetting the number of plates in this individual to the correct value
                setattr(individual, "number_of_plates_in_cassette_2A", len(getattr(individual, "material_matrix_cassette_2A")))

                new_child_ind.parent_string = "{},".format(parent_.ind_count)
                new_child_ind.allow_mutation = False

            if self.options['material_override_boolean']:
                new_child_ind.material_override()
    ...
    def build_and_run_mcnp_evaluation_input(self, individual,
        ...
        ### Building MCNP input file
        self.mcnp_file_handler.write_mcnp_input(template_file_str=template_file_,
                                                dictionary_of_replacements=data_dictionary_,
    ...
    def mutate(self, list_of_individuals, material_matrix_value, do_variable_size_mutation = False, mutating_debug = False):
        ...
        if self.options['mutation_type'] == 'bitwise':
            ...
            for ind_count, individual in enumerate(list_of_individuals):
                ...
                if individual.allow_mutation == False:
                    continue
    ...
    def wait_on_jobs_cnn_train(self, run_type, unique_flag = ""):
        waiting_on_jobs = True
        jobs_completed = 0
        total_time = 0
        jobs_to_be_waited_on = getattr(self, run_type + "_inputs")
    ...
    def write_options_funct(self, output_file, individual):
        ...
        for write_option in self.options['output_writeout_values']:
            ...
            if write_option == 'integral_keff_cnn':
                write_string += str(individual.integral_keff_cnn) + ","
