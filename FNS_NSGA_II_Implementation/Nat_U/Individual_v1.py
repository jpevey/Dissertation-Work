        ...
        for _ in self.material_matrix:
            for material in _:
                ...
                if str(material) == str(material_to_count):
                    if material_count <= self.options['number_of_4x_materials']:
                        material_multiplier = 4
                    mat_count += 1 * material_multiplier
        return mat_count

    def material_override(self):
        #print(self.material_matrix)

        for list_count, material_list in enumerate(self.material_matrix):
            #print(list_count, material_list)
            for override_list in self.options['material_override_list']:
                if list_count == override_list[0]:
                    #print("Found one!", override_list[1])
                    self.material_matrix[list_count] = override_list[1]

        try:
            for override_list in self.options['material_matrix_cassette_2A_override_list']:
                print('material_matrix_cassette_2A',getattr(self, 'material_matrix_cassette_2A'), len(self.material_matrix_cassette_2A) - override_list[0])
                self.material_matrix_cassette_2A[len(self.material_matrix_cassette_2A) - override_list[0] - 1] = override_list[1]
                print('material_matrix_cassette_2A', getattr(self, 'material_matrix_cassette_2A'), override_list[1])
                #print('material_matrix_cassette_2A', getattr(self, 'material_matrix_cassette_2A'))
                #self.material_matrix_cassette_2A[len(self.material_matrix_cassette_2A) - override_list[0]] = override_list[1]
        except:
            pass
        #print(self.material_matrix)
